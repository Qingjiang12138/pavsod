...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    ...
    def forward(self, srcs, pos_embeds):
        ...
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten,
                              mask_flatten, use_dpd=self.use_dpd)
...
class MSDeformAttnTransformerEncoder(nn.Module):
    ...
    def get_reference_points(self, spatial_shapes, valid_ratios, n_head, n_points, device, use_dpd=True):
        ...
        if not use_dpd:
            print("[forward] DPD: DISABLED (ablation)")
            reference_points = reference_points[:, :, None, :, None, :].repeat(1, 1, n_head, 1, 1, 1)
            return reference_points
        print("[forward] DPD: ENABLED")
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, use_dpd=True):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, 8, 4, device=src.device, use_dpd=use_dpd)
...
class MSDeformAttnPixelDecoder_Final(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            use_dpd: bool = True,
            use_audio: bool = True,
            use_av_sam: bool = True,
            use_av_iam: bool = True,
    ):
        """
        ...
        self.use_av_sam = use_av_sam
        self.use_av_iam = use_av_iam
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret["use_dpd"] = cfg.MODEL.PIXEL_DECODER.USE_DPD
        ret["use_audio"] = cfg.MODEL.PIXEL_DECODER.USE_AUDIO
        ret["use_av_sam"] = cfg.MODEL.PIXEL_DECODER.USE_AV_SAM
        ret["use_av_iam"] = cfg.MODEL.PIXEL_DECODER.USE_AV_IAM
        return ret
    ...
    def forward_features(self, features, audios, images):
        ...
        if self.use_audio:
            ...
            if not self.use_av_iam:
                audio_sep = None
        else:
            print("[forward] Audio: DISABLED (ablation)")
            out_av = out
            audio_sep = None
